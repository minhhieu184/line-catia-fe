...
export default function QuizPage() {
  ...
  const setQuizSpinDialogOpen = useCatiaStore(
    (state) => state.setQuizSpinDialogOpen
  );
  const quizSummaryDialogOpen = useCatiaStore(
    (state) => state.quizSummaryDialogOpen
  );
  const setQuizSummaryDialogOpen = useCatiaStore(
    (state) => state.setQuizSummaryDialogOpen
  );
  ...
  const currentQuestion = useMemo(() => {
    ...
    if (lang !== "en" && translation && translation?.choices?.length > 0)
      return translation;
    ...
    <main className="relative h-screen">
      <ScrollArea>
        <Container className="h-screen">
          {currentQuestion && (
            <div className="flex h-full flex-1 flex-col max-h-screen">
              <div className="grid grid-cols-3 pt-4 smH:pt-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-semibold">
                    {session.score >= 0 ? session.score : 0}
                  </span>
                </div>
              ...
              {game?.name && (
                <div className="flex items-center justify-center mt-1">
                  <div className="px-3 py-2 bg-bgFreebies rounded-2xl">
                    <GradientText className="font-bold tracking-wide text-lg leading-[1.25]">
                      {game?.name}
                    </GradientText>
                  </div>
                </div>
              ...
              <ScrollArea className="mt-5 flex-1 rounded-2xl bg-white/15 no-scrollbar relative min-h-40 smH:mt-2 smH:min-h-36">
                <div className="flex flex-1 shrink flex-col items-center gap-4 p-3 h-full">
              ...
              <div className="mt-5 flex flex-col gap-4">
                {currentQuestion?.choices?.length === 2 && (
                  <Button3D
                    disabled
                    className="h-auto rounded-full invisible min-h-[60px] smH:min-h-12"
                ...
                {currentQuestion?.choices?.map((choice) => (
                  <Button3D
                    key={choice.key}
                    onClick={() =>
                      session?.history[session?.next_step - 1]?.correct !==
                        false && setSelected(choice.key)
                    }
                    className={clsx(
                      "h-auto min-h-[60px] smH:min-h-12 rounded-full bg-[#5BB6D0]",
                      session?.history[session?.next_step - 1]?.correct ===
                    ...
                    contentClassName={clsx(
                      ...
                      session?.history[session?.next_step - 1]?.correct ===
                        false &&
                        session?.history[session?.next_step - 1]?.answer ===
                          choice.key &&
                        "border-2 border-[#F73131] bg-[#FFC8C8]",
                      session?.history[session?.next_step - 1]?.correct ===
                        false &&
                        session?.history[session?.next_step - 1]
                          ?.correct_answer === choice.key &&
              ...
              {session?.history[session?.next_step - 1]?.correct !== false ? (
                <div className="mt-4 grid grid-cols-2 gap-1.5 pb-6 smH:pb-3">
                  ...
                  >
                    ...
                    >
                      <LifeLineIcon />
                      <span className="ml-1.5 leading-none text-primary">
                        50:50
                      </span>
                ...
                <div className="w-full mt-4 pb-6 smH:pb-3">
                  ...
                  <Button
                    className="w-full"
                    size="sm"
                    onClick={() => setQuizSummaryDialogOpen(true)}
                  >
                    Finish
                  </Button>
                </div>
